...
Снимок на 2026-09-19. Источник правды по железу и пластику — Obsidian (`справочники/3d-печать.md`, `справочники/филаменты-инвентарь.md`).
...
SUNLU PLA Bone White, PLA Beige, PLA Matte Grey, PLA Matte Oak, PETG Olive Green — все 1.75 мм. PLA Bone White / Beige печатаются на калиброванном `SUNLU PLA @BBL A1M`, Matte Grey / Oak — на калиброванном `SUNLU Matte @BBL A1M` (таблица ниже); PETG — на `SUNLU PETG Olive Green @BBL A1M` (не калибровался).

## Пресеты Studio на диске

- Пользовательские: `SUNLU PLA @BBL A1M` и `SUNLU Matte @BBL A1M` (калибровки, `profiles/user/`); `SUNLU PETG Olive Green @BBL A1M` (клон `Generic PETG @BBL A1M` без изменений); `Gridfinity PETG 0.20 A1 mini` (`0.20mm Standard` + wall_loops 3, bottom 5, gyroid).
