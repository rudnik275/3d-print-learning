...
<name>.gcode.3mf): checks 1, 2, 5, 7, 10, 11, 12 of references/forecast.md, plus the start-G-code sanity
(M1002 count vs an un-indented `M109 S205` = Ender placeholder, 2026-09-17) and where the support sits by z.
...
def layers(path):
    ...
    out = []; cur = None; feat = None; x = y = 0.0; m73 = None; m73_at = None
    for line in open(path, errors="replace"):
...
def features_e(path):
    ...
    tot = defaultdict(float); feat = None; x = y = 0.0
    for line in open(path, errors="replace"):
        if line.startswith("; FEATURE: "): feat = line[11:].strip(); continue
...
def report(d):
    ...
    for o in p["objects"]:
        b = box(o); flags = []
        if b[0] < 20: flags.append("x<20 (левые 20 мм стола)")
        if max(b[3], b[4]) > BED or b[5] > BED: flags.append("OUTSIDE 180")
    ...
    print(f"  start gcode: M1002 x{m1002}, un-indented `M109 S205` x{s205} -> {'OK (Bambu)' if m1002 > 100 and s205 == 0 else 'WRONG: Ender placeholder?'}")
    ...
    print(f"  support {100 * sup / tot:.1f} % of E ({'>40 %: пересмотреть ориентацию' if sup / tot > 0.4 else 'ok'}); brim E {fe.get('Brim', 0):.0f} mm; features: " +
    ...
    model_e = [(l, sum(v for k, v in l["feat"].items() if not k.startswith("Support") and k not in ("Brim", "Skirt", "Custom", "Prime tower"))) for l in L]
    tot_e = sum(e for _, e in model_e) or 1.0
    ...
    for i, l in enumerate(L):
        if l["s"] is not None and l["s"] < slow: cur.append(i)
            ...
            cur = []
    if len(cur) >= 5: runs.append((L[cur[0]]["z"], L[cur[-1]]["z"], len(cur), min(L[k]["s"] for k in cur)))
    print(f"  layers < {slow:.0f} s (slow_down_layer_time), runs >= 5: " + ("; ".join(f"z {a:.1f}-{b:.1f} ({n} слоёв, min {s:.1f} с)" for a, b, n, s in runs) if runs else "—"))
    ...
    print("  E jumps >= 2x with walls above (линия пола?): " + ("; ".join(f"z {a:.1f}{'' if a == b else f'-{b:.1f}'} (x{k:.1f})" for a, b, k in merged) if merged else "—"))
