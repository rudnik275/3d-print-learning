...
Arcs G2/G3 are counted. Forecast check 16 in references/forecast.md.

Thunderstorm whistle 19.09: 0.12 → 830 hops / 3.4 m, 0.08 → 1444 hops / 5.6 m, ~4 hops per layer across
the six windows either way; the 0.12 print whistled weakly with strings inside."""
import re, math, sys
...
if __name__ == "__main__":
    ...
    bands = [(0, zmax, "весь объект")]
    if "--bands" in a:
        bands = []
        for b in a[a.index("--bands") + 1].split(","):
            z0, z1, lab = b.split(":"); bands.append((float(z0), float(z1), lab))
    print(f"{a[0]}: {len(layers)} слоёв, ретрактов {sum(L['retr'] for L in layers)}")
    print(f"{'полоса':34s} {'слоёв':>5s} {'ретр/сл':>7s} {'воздух/сл':>9s} {'мм/сл':>6s} {'воздух всего':>12s} {'мм всего':>8s}")
    tn = 0; tmm = 0.0
    for lo, hi, lab in bands:
        ...
        print(f"{lab:34s} {len(sel):5d} {sum(L['retr'] for L in sel) / len(sel):7.1f} {n / len(sel):9.1f} {mm / len(sel):6.1f} {n:12d} {mm:8.0f}")
    print(f"итого через воздух внутри детали: {tn} переездов, {tmm / 1000:.1f} м")
    if "--png" in a:
